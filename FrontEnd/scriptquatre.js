...
fetch(apiWorks)
    ...
    .then( jsonlistPhotos => {
        ...
        console.log("les photos :",getPhotos) /* affiche les éléments dans la console */
...
function afficherPhotos(array,sectionGallery, isModal = false) {
    ...
    for (let i = 0; i < array.length; i++) { /* boucle for pour itéré chaque élément */
        ...
        if(isModal){ /* Si modal, on ajout l'icon trash */
    ...
    sectionGallery.appendChild(figurePhotos)/* on ajoute tout dans le conteneur */
...
const openModal = function (e){  /* utilisé comme gestionnaire d'évènements quand il y aun clic sur un élément avec "js-modal" */
    e.preventDefault()     /* empêché un rechargement de la page */

    const target = document.querySelector(e.target.getAttribute('href')) /* récupère élément cible du modal grace a alvaleur de l'attribut href */
        ...
        if (modal){
            modal.addEventListener('click',closeModal) /* écouteur d'évenements pour ouvrir ou fermer la modal */
...
const closeModal = function (e) { /* fermer la modal */
    ...
    modal.style.display = 'none' /* on fait l'inverse de l'ouverture de la modale */
...
async function afficherGaleriePhoto() {
    ...
    .then(data => data.json())
    .then(jsonlistPhotos => {
        getPhotos = jsonlistPhotos /* liste de photos */
...
function supprimerPhoto(photoId, index) { 
    ...
    modalPhoto.removeChild(modalTrash.parentElement)/* Supprimer photo de la galerie modale */
    ...
    galleryPhoto.parentElement.removeChild(galleryPhoto);/* Supprimer photo de la page principale */
    ...
    .then(response => {
        if (response.ok) {
            ...
            console.error("La photo n'a pas été supprimé de la base de données.")
        }
    })
    .catch(error => {
...
function hideCreateForm() {
    ...
    modalCloseSpan.style.display = 'none'
     /* apppelé pour rétablir l'affichage de la modal */
}
...
function createForm(modalWrapper, getCategories) {
    ...
    modalBackBtn.addEventListener("click", function (){
        ...
        showAllModalContent()
        const fakeEvent = {
            preventDefault: function () {},  // Fonction bidon pour simuler preventDefault
            target: document.querySelector('.js-modal'),  // Cible similaire à celle du clic sur la target
        };
        openModal(fakeEvent);
    });
        /* ICI PROBLEME , causé par les paramètres ?*/
    ...
    const ModalBtnAdd = document.createElement("p") /* Btn d'ajout de photo, déclenche le sélecteur de fichires */
    ...
    const modalTitleLabel = document.createElement("label") /* champ d'entrée de type de fichier pour selectionner une img */
    ...
    modalCategorieSelect.classList.add("modal-hide-text") /* MAIS n'affiche pas la catégorie apres l'avoir sélectionné */
...
async function sendFormData(e,modalImageInput) {
    ...
    const apiWorks = "http://localhost:5678/api/works" /* récup URL */
    const token = localStorage.getItem('token')        /* récup Token */
   ...
   let catId = modalCategorieSelect.value
    ...
    if (modalCategorieSelect.value !== "") {
        ...
        // Ajoutez votre logique ici pour traiter la sélection de la catégorie
    } else {
    ...
    try {
        ...
        if (response.ok) { /* Vérification de la répose du serveur */
